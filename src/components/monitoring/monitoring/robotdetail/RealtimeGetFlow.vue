    ...
    methods: {
        ...
        realtimeData() {
            this.interval = setInterval(() => {
                ...
                let tempCmd = [];
                let tempFeedback = [];
                this.$http.post(`${this.baseUrl}/robotdetails/robotinfo/trend/flow`, { factoryid: this.factoryid, boothid: this.boothid, zoneid: this.zoneid, robotid: this.robotid }).then((result) => {
                    for (let i = 0; i < result.data.length; i++) {
                        let date = new Date(result.data[i].date_trunc);
